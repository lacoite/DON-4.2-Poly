...
{
	FString PurgedXml = Xml;

	int32 StartIndex = PurgedXml.Find(TEXT("<"), ESearchCase::CaseSensitive, ESearchDir::FromStart);
	if (StartIndex > 0)
	{
		PurgedXml = PurgedXml.RightChop(StartIndex);
	}
	int32 Length = PurgedXml.Find(TEXT(">"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
	if (Length >= 0)
	{
		PurgedXml = PurgedXml.Left(Length + 1);
	}

	if (PurgedXml.Contains(TEXT("<![CDATA["), ESearchCase::CaseSensitive, ESearchDir::FromStart))
	{
		PurgedXml = ReplaceCDATA(PurgedXml);
	}
